...
// SecureType sends the given UTF-8 bytes as synthetic keypresses.
func SecureType(b []byte) {
	...
	for len(b) > 0 {
		...
		if shift {
			C.postKey(56, true) // Left Shift down
		}
		...
		if shift {
			C.postKey(56, false) // Left Shift up
		}
...
func needsShift(r rune) bool {
	return (r >= 'A' && r <= 'Z') || strings.Contains("!@#$%^&*()_+{}|:\"<>?~", string(r))
}
